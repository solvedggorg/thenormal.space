...
import { randomId, randomToken, sha256Bytes, toBase64Url } from "../../src/lib/crypto";
import { logJson } from "../../src/lib/security";
...
export function jumpcloudEnabled(env: Cloudflare.Env): boolean {
...
export function issuerOf(env: Cloudflare.Env): string {
  return (env.JUMPCLOUD_ISSUER || "https://oauth.id.jumpcloud.com").replace(/\/$/, "");
}

export function redirectUriOf(env: Cloudflare.Env, url: string): string {
  if (env.JUMPCLOUD_REDIRECT_URI) return env.JUMPCLOUD_REDIRECT_URI;
  return `${new URL(url).origin}/oidc/callback`;
}

export async function readJumpCloudSession(c: JcCtx): Promise<JumpCloudSession | null> {
...
export function writeJumpCloudCookie(c: JcCtx, sessionId: string): void {
...
export function clearJumpCloudCookie(c: JcCtx): void {
  deleteCookie(c, JC_COOKIE, { path: "/" });
}

export async function startJumpCloudLogin(c: JcCtx): Promise<Response> {
...
export async function finishJumpCloudLogin(c: JcCtx): Promise<Response> {
  ...
  if (!identity) {
    logJson("security", { event: "jumpcloud.denied" });
    return c.text("JumpCloud did not accept this sign-in.", 403);
  }
  const sessionId = randomId();
  await c.env.KV.put(
    `jc:sess:${sessionId}`,
    JSON.stringify({ email: identity.email, name: identity.name, exp: Math.floor(Date.now() / 1000) + JC_SESSION_TTL_SEC }),
    { expirationTtl: JC_SESSION_TTL_SEC },
  );
  writeJumpCloudCookie(c, sessionId);
  logJson("info", { event: "jumpcloud.login", email: identity.email });
  return c.redirect(saved.next || "/", 302);
}

export async function destroyJumpCloudSession(c: JcCtx): Promise<void> {
  const id = getCookie(c, JC_COOKIE);
...
async function exchangeJumpCloud(
  ...
  if (env.TEST_JUMPCLOUD_USER) {
    return { email: env.TEST_JUMPCLOUD_USER, name: "JumpCloud" };
  }
  ...
  if (!tokenRes.ok) {
    logJson("error", { event: "jumpcloud.token", status: tokenRes.status });
    return null;
  }
