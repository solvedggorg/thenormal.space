...
output "access_analytics_aud" {
  value = try(cloudflare_zero_trust_access_application.analytics[0].aud, null)
}
...
output "wrangler_bindings" {
  value = {
    ...
    d1_analytics_id      = cloudflare_d1_database.this["analytics"].id
    ...
    kv_analytics_id      = cloudflare_workers_kv_namespace.this["analytics"].id
    r2_media             = cloudflare_r2_bucket.media.name
    r2_analytics         = cloudflare_r2_bucket.analytics.name
    queue_name           = cloudflare_queue.shop_events.queue_name
    analytics_queue_name = cloudflare_queue.analytics_events.queue_name
    ...
    analytics_policy_aud = try(cloudflare_zero_trust_access_application.analytics[0].aud, "")
